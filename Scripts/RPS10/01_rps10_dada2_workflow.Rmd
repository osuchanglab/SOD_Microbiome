---
title: "RPS10 DADA2 Workflow"
output: html_document
date: "2024-06-26"

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
knitr::opts_knit$set(root.dir = "~/SOD_DADS_microbiome/Data/RPS10")
...
out <- filterAndTrim(cutFs, filtFs, cutRs, filtRs, maxN = 0, maxEE = c(5, 5), truncQ = 5,
    minLen = 150, maxLen = Inf, rm.phix = TRUE, compress = TRUE, multithread = TRUE) 
head(out)
...
write.table(seqtab.nochim, file="dada2_outputs/rps10_seqtab_nochim.out",sep="\t",quote=F)
```
...
write.table(track, file="dada2_outputs/rps10_trackreads.out",sep="\t",quote=F)
```

### If everything look good, let's assign taxonomy
``` {r assignTaxonomy}
set.seed(1)
oomycetedb.ref <- "metadata_dbs/rps10_reference_db.fa"  # CHANGE ME to location on your machine
taxa <- assignTaxonomy(seqtab.nochim, oomycetedb.ref, multithread = TRUE, minBoot = 0, outputBootstraps = TRUE, verbose=TRUE, tryRC = FALSE)

taxa.print <- taxa  # Removing sequence rownames for display only
rownames(taxa.print) <- NULL

write.table(taxa, file="dada2_outputs/rps10_taxa.out",sep="\t",quote=F)
```
...
set.seed(1)
oomycetedb.ref <- "metadata_dbs/rps10_reference_db.fa"  # CHANGE ME to location on your machine
taxa <- assignTaxonomy(seqtab.nochim, oomycetedb.ref, multithread = TRUE, minBoot = 0, outputBootstraps = TRUE, verbose=TRUE, tryRC = TRUE)

taxa.print <- taxa  # Removing sequence rownames for display only
rownames(taxa.print) <- NULL

write.table(taxa, file="dada2_outputs/rps10_taxa_tryRC.out",sep="\t",quote=F)
```

### If everything look good, let's assign taxonomy but with minBoot set to 60
``` {r assignTaxonomy with minboot at 60 no RC}
set.seed(1)
oomycetedb.ref <- "metadata_dbs/rps10_reference_db.fa"  # CHANGE ME to location on your machine
taxa <- assignTaxonomy(seqtab.nochim, oomycetedb.ref, multithread = TRUE, minBoot = 0, verbose=TRUE, tryRC = FALSE)

taxa.print <- taxa  # Removing sequence rownames for display only
rownames(taxa.print) <- NULL

write.table(taxa, file="dada2_outputs/rps10_taxa_minboot60.out",sep="\t",quote=F)
```

### If everything look good, let's assign taxonomy but with minBoot set to 60, with RC set
``` {r assignTaxonomy with minboot at 60}
set.seed(1)
oomycetedb.ref <- "metadata_dbs/rps10_reference_db.fa"  # CHANGE ME to location on your machine
taxa <- assignTaxonomy(seqtab.nochim, oomycetedb.ref, multithread = TRUE, minBoot = 60, verbose=TRUE, tryRC = TRUE)

taxa.print <- taxa  # Removing sequence rownames for display only
rownames(taxa.print) <- NULL

write.table(taxa, file="dada2_outputs/rps10_taxa_minboot60_RC.out",sep="\t",quote=F)
```
